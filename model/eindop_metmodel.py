...
def collect_data(grid, step):
    """
    Collect data from the current grid state to train the predictive model.
    """
    step_data = []
...
# Modify `simulation_step` to collect data after each step
...
def train_rl_agent(grid, agent):
    # nteracts with the grid and learns based on state transitions
...
#markov_model = train_markov_model(grid)
#rl_agent = RLAgent(action_space=[0, 1, 2], state_space=[0, 1, 2])  # Example actions and states
...
for step in range(10):  # Run simulation for 10 steps
    ...
    # Prediction using Markov Model
    #predict_next_step_markov(grid, markov_model)
    # Reinforcement Learning interaction
    #strain_rl_agent(grid, rl_agentz
    # Optionally visualize results
    # visualize_blood_vessel_and_mutations(grid, blood_vessel_grid, cell_types, distance_grid, step)
